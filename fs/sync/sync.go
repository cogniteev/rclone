...
func newSyncCopyMove(ctx context.Context, fdst, fsrc fs.Fs, deleteMode fs.DeleteMode, DoMove bool, deleteEmptySrcDirs bool, copyEmptySrcDirs bool, listFiles string) (*syncCopyMove, error) {
...
//
// dir is the start directory, "" for root
func runSyncCopyMove(ctx context.Context, fdst, fsrc fs.Fs, deleteMode fs.DeleteMode, DoMove bool, deleteEmptySrcDirs bool, copyEmptySrcDirs bool, listFiles string) error {
	ci := fs.GetConfig(ctx)
	...
	if deleteMode == fs.DeleteModeBefore {
		...
		do, err := newSyncCopyMove(ctx, fdst, fsrc, fs.DeleteModeOnly, false, deleteEmptySrcDirs, copyEmptySrcDirs, listFiles)
		if err != nil {
			return err
		}
		err = do.run()
	...
	do, err := newSyncCopyMove(ctx, fdst, fsrc, deleteMode, DoMove, deleteEmptySrcDirs, copyEmptySrcDirs, listFiles)
	if err != nil {
		return err
	}
	return do.run()
...
// CopyFiles copies list of files from fsrc into fdst
func CopyFiles(ctx context.Context, fdst, fsrc fs.Fs, listFiles string) error {
	return runSyncCopyMove(ctx, fdst, fsrc, fs.DeleteModeOff, false, false, false, listFiles)
}
